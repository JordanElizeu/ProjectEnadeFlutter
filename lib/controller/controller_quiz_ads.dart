...
class ControllerQuizAds extends GetxController{
  ...
  _validatorQuestionsResultAds(){
    int response = ControllerQuestions().getResponse(_questionsAds,showQuestionScreenAds);
    if(response == _questionSelectedRadiusAds){
      _numberCorrectQuestionsAds ++;
    }else{
      listIncorrectQuestionsAds.add("Questão: $showQuestionScreenAds");
      listIncorrectQuestionsAds.add("Resposta: ${_questionsAds[showQuestionScreenAds]["response"]}");
    }
  }
